...
public class Solution {
    ...
    private void addOperatorsHelper(List<String> res, StringBuilder sb, String num, int target, int p, long value, long multed) {
        if (p == num.length()) {
    ...
    public List<Integer> majorityElement(int[] nums) {
        ...
        for (int i : nums) {
            if (i == n1) count1 += 1;
            else if (i == n2) count2 += 1;
        }
        if (count1 > len / 3) res.add(n1);
        if (count2 > len / 3) res.add(n2);
        return res;
    ...
    private List<int[]> divideAndconquer(int[][] buildings, int start, int end) {
        ...
        while(!left.isEmpty() || !right.isEmpty()) {
            ...
            if(leftx < rightx) {
                ...
                x = left.get(0)[0];
                leftH = left.get(0)[1]; left.remove(0);
                rightH = right.get(0)[1]; right.remove(0);
            }
    ...
    private void combiSum3Helper(List<List<Integer>> res, List<Integer> sumList, int k, int n, int cur, int sum) {
        if (sum > n || sumList.size() > k) return;
        if (sumList.size() == k) {
            if (sum == n)
    ...
    public int rob(int[] nums) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            else dp[i + 1] = Math.max(dp[i], dp[i - 1] + nums[i]);
        }
        int temp = dp[n];
        dp = new int[n + 1];
        for (int i = 1; i < n; i++) {
    ...
    private void dfs(int x, int y, List<String> res, Trie root, char[][] board, StringBuilder sb) {
        ...
        char c = board[x][y];
        if (root.nextLevel[c - 'a'] == null) return;
        ...
        if (x > 0) dfs(x - 1, y, res, next, board, sb);
        if (y > 0) dfs(x, y - 1, res, next, board, sb);
        if (x < board.length - 1) dfs(x + 1, y, res, next, board, sb);
        if (y < board[0].length - 1) dfs(x, y + 1, res, next, board, sb);
    ...
    private boolean findOrderHelper(int c, List<List<Integer>> graph, Set<Integer> removed, boolean[] visited, int[] res) {
        if (removed.contains(c)) return true;
